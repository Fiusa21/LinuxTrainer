"""
Working Wahoo Kickr Smart Trainer implementation
Based on actual debug data from KICKR CORE A62A
"""
import asyncio
import struct
...
class KickrTrainerWorking(BaseDevice):
    """Working Wahoo Kickr Smart Trainer device"""
    ...
    def _parse_kickr_data(self, data: bytearray) -> Optional[PowerData]:
        """Parse Kickr data based on actual debug output"""
        if len(data) < 8:
            logger.warning(f"Insufficient data: {len(data)} bytes")
            return None
        
        try:
            # Based on debug output: 340000007b5db6cc0000799cb6fd450e
            # The structure appears to be:
            # Bytes 0-1: Flags (0x3400)
            # Bytes 2-3: Instantaneous Power (little-endian, signed)
            # Bytes 4-5: Some other data (not cadence based on values)
            # Bytes 6-7: Speed or distance data
            # Bytes 8-15: Additional data
            ...
            # For now, let's try to extract reasonable values
            # The cadence values from debug were way too high, so let's try different approaches
            
            if len(data) >= 6:
                # Try to find cadence in different positions
                # Based on the debug output, the values in positions 4-5 and 6-7 are too high
                # Let's try to find a reasonable cadence value
                
                # Check if there's a reasonable cadence value in the data
                for i in range(4, min(len(data) - 1, 12), 2):
                    if i + 1 < len(data):
                        ...
                        if 0 <= candidate <= 200:
                            cadence = candidate
                            break
                        # Or try dividing by 100 (in case it's scaled)
                        elif 0 <= candidate / 100 <= 200:
                            cadence = candidate / 100
                            break
            
            # Calculate speed based on power (rough estimation)
            # This is a simplified approach - real speed would need wheel revolution data
